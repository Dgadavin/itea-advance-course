...
class Ec2Inventory(object):
    ...
    def read_settings(self):
        ...
        if config.has_option('ec2', 'instance_filters') or 'EC2_INSTANCE_FILTERS' in os.environ:
            filters = os.getenv(
                ...
                config.get('ec2', 'instance_filters') if config.has_option('ec2', 'instance_filters') else ''
            )
    ...
    def include_rds_clusters_by_region(self, region):
        if not HAS_BOTO3:
            self.fail_with_error(
                "Working with RDS clusters requires boto3 - please install boto3 and try again", "getting RDS clusters"
            )
    ...
    def fail_with_error(self, err_msg, err_operation=None):
        ...
        if err_operation:
            err_msg = 'ERROR: "{err_msg}", while: {err_operation}'.format(err_msg=err_msg, err_operation=err_operation)
    ...
    def add_rds_instance(self, instance, region):
        ...
        if self.group_by_rds_parameter_group:
            ...
            if self.nested_groups:
                self.push_group(
                    self.inventory, 'rds_parameter_groups',
    ...
    def add_elasticache_cluster(self, cluster, region):
        ...
        if self.group_by_elasticache_parameter_group:
            self.push(
                self.inventory,
                self
                .to_safe("elasticache_parameter_group_" + cluster['CacheParameterGroup']['CacheParameterGroupName']),
                dest
            )
            if self.nested_groups:
                self.push_group(
                    self.inventory, 'elasticache_parameter_groups',
        ...
        if self.group_by_elasticache_replication_group and 'ReplicationGroupId' in cluster and cluster[
            'ReplicationGroupId']:
            self.push(
                self.inventory, self.to_safe("elasticache_replication_group_" + cluster['ReplicationGroupId']), dest
            )
            if self.nested_groups:
                self.push_group(
                    self.inventory, 'elasticache_replication_groups', self.to_safe(cluster['ReplicationGroupId'])
                )
    ...
    def get_instance_route53_names(self, instance):
        ...
        instance_attributes = ['public_dns_name', 'private_dns_name', 'ip_address', 'private_ip_address']
